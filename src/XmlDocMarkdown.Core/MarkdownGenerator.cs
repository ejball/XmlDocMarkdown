...
namespace XmlDocMarkdown.Core
{
	public sealed class MarkdownGenerator
	{
		...
		public IReadOnlyList<NamedText> GenerateOutput(XmlDocAssembly xmlDocAssembly)
		{
			return DoGenerateOutput(xmlDocAssembly).ToList();
		}

		public static string GetCodeGenComment(string assemblyName) => $"<!-- DO NOT EDIT: generated by xmldocmd for {assemblyName} -->";

		private IEnumerable<NamedText> DoGenerateOutput(XmlDocAssembly xmlDocAssembly)
		{
			var assembly = xmlDocAssembly.Info;
			...
			var publicTypes = xmlDocAssembly
				.Info
				.ExportedTypes
			...
			var publicTypesByNamespace = publicTypes
			...
			yield return CreateNamedText(GetAssemblyUriName(assembly) + ".md", writer =>
			{
				...
				string namespaceCountText = publicTypesByNamespace.Count == 1 ? "1 namespace" : $"{publicTypesByNamespace.Count} namespaces";
				int typeCount = publicTypesByNamespace.SelectMany(x => x.Types).Count();
				...
				foreach (var group in publicTypesByNamespace)
				{
					writer.WriteLine();
					...
					foreach (var typeInfo in group.Types)
					{
						...
						string summaryText = GetShortSummaryMarkdown(xmlDocAssembly, typeInfo.TypeInfo, context: new MarkdownContext(xmlDocAssembly.Info));
			...
			{
				yield return WriteMemberPage($"{GetNamespaceUriName(publicType.Namespace)}/{publicType.Path}", publicType.TypeInfo, xmlDocAssembly, assemblyFileName);

				var typeKind = GetTypeKind(publicType.TypeInfo);
				if (typeKind == TypeKind.Class || typeKind == TypeKind.Struct || typeKind == TypeKind.Interface)
				{
					var memberGroups = publicType.TypeInfo
						.DeclaredMembers
						.Where(IsPublic)
		...
		{
			return GetSummary(xmlDocAssembly.FindMember(member), member);
		}
		...
		{
			return memberInfo is TypeInfo ? GetTypeUriName((TypeInfo) memberInfo) : GetShortName(memberInfo);
		}
		...
		private NamedText WriteMemberPage(string path, MemberInfo memberInfo, XmlDocAssembly xmlDocAssembly, string assemblyFileName)
		{
			return WriteMemberPage(path, new[] { memberInfo }, xmlDocAssembly, assemblyFileName);
		}

		private NamedText WriteMemberPage(string path, IReadOnlyList<MemberInfo> memberGroup, XmlDocAssembly xmlDocAssembly, string assemblyFileName)
		{
			return CreateNamedText(path, writer =>
			{
				writer.WriteLine($"# {GetMemberGroupHeading(memberGroup)}");

				foreach (var memberInfo in memberGroup)
				{
					var context = new MarkdownContext(memberInfo);
					...
					{
						writer.WriteLine();
						writer.WriteLine($"# {GetMemberHeading(memberInfo)}");
					}

					var xmlDocMember = xmlDocAssembly.FindMember(memberInfo);

					var summary = GetSummary(xmlDocMember, memberInfo);
					if (summary != null && summary.Count != 0)
					{
						writer.WriteLine();
						writer.WriteLines(ToMarkdown(summary, context));
					}
					...
					if (xmlDocMember != null)
					{
						...
						if (typeParameters.Count + parameters.Count > 0)
						{
							...
							{
								string description = ToMarkdown(typeParameter.Description.FirstOrDefault()?.Inlines, context) ?? "";
								writer.WriteLine($"| `{typeParameter.Name}` | {description} |");
							}
							foreach (var parameter in parameters)
							{
								string description = ToMarkdown(parameter.Description.FirstOrDefault()?.Inlines, context) ?? "";
					...
					else if (typeKind == TypeKind.Class || typeKind == TypeKind.Struct || typeKind == TypeKind.Interface)
					{
						...
						if (innerMemberGroups.Count != 0)
						{
							...
							foreach (var innerMemberGroup in innerMemberGroups)
							{
								...
								string summaryText = GetShortSummaryMarkdown(xmlDocAssembly, firstInnerMember, context);
					...
					if (returnValue != null && returnValue.Count != 0)
					{
						...
						writer.WriteLine();
						writer.WriteLines(ToMarkdown(returnValue, context));
					}
					...
					if (propertyValue != null && propertyValue.Count != 0)
					{
						...
						writer.WriteLine();
						writer.WriteLines(ToMarkdown(propertyValue, context));
					}
					...
					if (remarks != null && remarks.Count != 0)
					{
						...
						writer.WriteLine();
						writer.WriteLines(ToMarkdown(remarks, context));
					}
					...
					if (examples != null && examples.Count != 0)
					{
						...
						writer.WriteLine();
						writer.WriteLines(ToMarkdown(examples, context));
					}
		...
		private static string GetMemberHeading(MemberInfo memberInfo)
		{
			return GetMemberGroupHeading(new[] { memberInfo });
		}

		private static string GetMemberGroupHeading(IReadOnlyList<MemberInfo> membersInfos)
		{
			return $"{GetFullMemberName(membersInfos[0])} {GetMemberGroupNoun(membersInfos)}";
		}
		...
		private static string GetFullTypeName(TypeInfo typeInfo)
		{
			string name = GetShortName(typeInfo);
			if (typeInfo.DeclaringType != null)
				name = $"{GetFullTypeName(typeInfo.DeclaringType.GetTypeInfo())}.{name}";
			return name;
		}
			...
			if (typeKind == TypeKind.Class || typeKind == TypeKind.Struct || typeKind == TypeKind.Interface)
			{
				...
				foreach (var baseInterface in baseInterfaces)
				{
					if (!(typeKind == TypeKind.Class && baseInterface.IsAssignableFrom(typeInfo.BaseType.GetTypeInfo())) &&
						!baseInterfaces.Any(x => x != baseInterface && baseInterface.IsAssignableFrom(x)))
					{
		...
		private static string ToMarkdown(XmlDocInline inline, MarkdownContext context)
		{
			...
			MemberInfo see = inline.See;
			var seeType = see as TypeInfo;

			if (see != null && text.Length == 0)
				text = GetShortName(see);

			if (text.Length != 0)
			{
				if (inline.IsCode || see != null)
					text = $"`{text}`";

				if (inline.IsParamRef || inline.IsTypeParamRef)
					text = $"*{text}*";

				if (see != null && context.Assembly == GetMemberAssembly(see) && seeType != context.Type?.GetTypeInfo() && see != context.Member)
				{
					string path = "";
					...
					if (context.Member != null)
					{
						if (seeType != null)
						{
							if (seeType.Namespace == context.Type.Namespace)
								path = $"../{GetTypeUriName(seeType)}.md";
							else
								path = $"../../{GetNamespaceUriName(seeType.Namespace)}/{GetTypeUriName(seeType)}.md";
						}
						else
						{
							if (see.DeclaringType == context.Type)
								path = $"{GetMemberUriName(see)}.md";
							else if (see.DeclaringType?.Namespace == context.Type.Namespace)
								path = $"../{GetTypeUriName(see.DeclaringType.GetTypeInfo())}/{GetMemberUriName(see)}.md";
							else
								path = $"../../{GetNamespaceUriName(see.DeclaringType?.Namespace)}/{GetTypeUriName(see.DeclaringType.GetTypeInfo())}/{GetMemberUriName(see)}.md";
						}
					}
					else if (context.Type != null)
					{
						...
						{
							if (see.DeclaringType?.Namespace == context.Type.Namespace)
								path = $"{GetTypeUriName(see.DeclaringType.GetTypeInfo())}/{GetMemberUriName(see)}.md";
							else
								path = $"../{GetNamespaceUriName(see.DeclaringType?.Namespace)}/{GetTypeUriName(see.DeclaringType.GetTypeInfo())}/{GetMemberUriName(see)}.md";
						}
		...
		private static Assembly GetMemberAssembly(MemberInfo memberInfo)
		{
			return (memberInfo as TypeInfo ?? memberInfo.DeclaringType.GetTypeInfo())?.Assembly;
		}

		private class MarkdownContext
		{
			public MarkdownContext(Assembly assembly)
			{
				Assembly = assembly;
			}

			public MarkdownContext(MemberInfo member)
			{
				Assembly = GetMemberAssembly(member);
