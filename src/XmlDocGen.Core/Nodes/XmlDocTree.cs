...
public sealed class XmlDocTree
{
	...
	public XmlDocTree(IEnumerable<XmlDocAssemblyNode> assemblies)
	{
		...
		m_nodesByRef = Assemblies.SelectMany(x => x.EnumerateNodes()).GroupBy(x => x.Ref).ToDictionary(x => x.Key, x => x.First());
		m_nodesByMember = Assemblies.SelectMany(x => x.EnumerateNodes()).Where(x => x.MemberInfo is not null).GroupBy(x => x.MemberInfo!).ToDictionary(x => x.Key, x => x.First());
	}

	/// <summary>Creates a tree from reflected assemblies and XML documentation files.</summary>
	public XmlDocTree(IEnumerable<(Assembly Assembly, XmlDocXmlFile Xml)> inputs)
		: this(inputs.Select(x => new XmlDocAssemblyNode(x.Assembly, x.Xml)))
	{
	}
	...
	public IEnumerable<XmlDocNode> EnumerateNodes(XmlDocNodeVisibility visibility) => Assemblies.SelectMany(x => x.EnumerateNodes(visibility));
